...
invoke_formatter() {
  ...
  case ${formatter} {
    clang)
      ...
      local formatter_output=$(${formatter} --version)
      local formatter_version
      local token
      for token in ${(z)formatter_output}; do
        if [[ ${token} == <->.<->.<-> ]] {
          formatter_version=${token}
          break
        }
      done

      if [[ -z ${formatter_version} ]] {
        log_error "Unable to determine clang-format version from: ${formatter_output}"
        exit 2
      }
      ...
      if is-at-least 20.0.0 ${formatter_version}; then
        log_error "clang-format major version must be 19 (found ${formatter_version})."
        exit 2
      fi
